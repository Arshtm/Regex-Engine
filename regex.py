...
def plain_check(regex, text):
    ...
    if len(regex.replace('$', '').replace('\\', '')) > len(text):
        ...
        return check_one_by_one(regex, text) or plain_check(regex, text[1:])
...
def check_regex_with_question_mark(regex, text):
    ...
    return plain_check(regex_exclude, text) or plain_check(regex_include, text)
